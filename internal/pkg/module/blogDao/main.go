...
import (
	...
	"baby-fried-rice/internal/pkg/module/blogDao/cache"
	...
	"baby-fried-rice/internal/pkg/module/blogDao/service/application"
	"crypto/tls"
	"fmt"
	"time"
)
...
func ServerRun() {
	...
	svr := rpc.NewServerGRPC(fmt.Sprintf("%v:%v", conf.Server.RPCServer.Addr, conf.Server.RPCServer.Port),
		log.Logger, &cert)
	blog.RegisterDaoBlogServer(svr.GetRpcServer(), &application.BlogService{})
