...
import (
	...
	"baby-fried-rice/internal/pkg/module/blog/server"
	"crypto/x509"
	"fmt"
...
func GetBlogClient() (blog.DaoBlogClient, error) {
	cli, err := GetClientGRPC(config.GetConfig().Rpc.SubServers.BlogDaoServer)
	if err != nil {
		log.Logger.Error(err.Error())
		return nil, err
	}
	return blog.NewDaoBlogClient(cli.GetRpcClient()), nil
}
