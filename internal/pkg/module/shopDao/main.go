...
import (
	...
	"baby-fried-rice/internal/pkg/module/shopDao/cache"
	...
	"baby-fried-rice/internal/pkg/module/shopDao/service/application"
	"crypto/tls"
	"fmt"
	"time"
)
...
func ServerRun() {
	...
	svr := rpc.NewServerGRPC(fmt.Sprintf("%v:%v", conf.Server.RPCServer.Addr, conf.Server.RPCServer.Port),
		log.Logger, &cert)
	shop.RegisterDaoShopServer(svr.GetRpcServer(), &application.ShopService{})
